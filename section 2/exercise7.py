...
print(f"Lottery #s:  {lottery_numbers}")
...
for player in players:
    numbers_matched = len(set(player['numbers'].intersection(lottery_numbers))) # Calculate how many numbers they matched
    if numbers_matched > len(top_player["numbers"].intersection(lottery_numbers)):   # If they matched more than the current top player...
        top_player = player

winnings = 100 ** len(top_player["numbers"].intersection(lottery_numbers))
winning_count = len(top_player["numbers"].intersection(lottery_numbers))

print(f"{top_player['name']} won {winnings} with {winning_count} matching numbers.")
